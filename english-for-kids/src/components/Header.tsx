...
export const Header: React.FC = () => {
  ...
  const { isShowLeftMenu, isModePlay } = useTypedSelector(
    (state) => state.cards
  );
    ...
    <>
      {pathname.includes(ADMIN) ? (
        <div className="header-admin">
          ...
          <NavLink
            to="/"
            className="button button-transparent text-title"
        ...
        <header className="header" onClick={valueFalseLeftMenu}>
          <div className="header-container">
            <div
              className={`header-burger ${isShowLeftMenu && "active"}`}
              id="header-burger"
              onClick={viewLeftMenu}
            >
