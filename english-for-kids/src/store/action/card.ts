...
import {
  ...
  UPDATE_CATEGORY,
  VIEW_LEFT_MENU,
} from '../reducers/cardsReducer.module';
...
export function clearCategories(arr: Stars[]): IAction {
  return { type: CHANGE_ARR_STARS, payload: arr };
}
...
export function updateCard(
  ...
  return async (dispatch: Dispatch<IAction>) => {
    try {
      ...
      dispatch({
        type: UPDATE_CARD,
        payload: { indexCategory, indexCard, data },
      });
    } catch (e) {
      throw Error(e);
    }
  ...
  return async (dispatch: Dispatch<IAction>) => {
    try {
      ...
      dispatch({
        ...
        payload: { indexCategory, nameCategory },
      });
    } catch (e) {
      throw Error(e);
    }
