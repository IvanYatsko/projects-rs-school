...
import {
  ...
  StatisticTitle,
} from './statisticReducer.module';

export function createArrField(): IFieldItem[] {
  const savedStatistics: IFieldItem[] = JSON.parse(
    localStorage.getItem('arrStatistic') || '[]'
  ) as IFieldItem[];
...
export const statisticReducer = (
  state: IlistStatistic = listStatistic,
  action: IAction
): IlistStatistic => {
  switch (action.type) {
    ...
    case SORT_STATISTIC_FIELD:
      ...
      if (state.sort[key]) {
        if (
          key === StatisticTitle.CATEGORY ||
          key === StatisticTitle.TRANSLATION ||
          key === StatisticTitle.WORD
        ) {
          sortArray = state.field.sort((a, b) =>
            a[key] < b[key] ? -1 : a[key] > b[key] ? 1 : 0
          );
        } else {
          sortArray = state.field.sort(
